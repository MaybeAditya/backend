...
// Date-range adherence (Admin only)
app.get('/adherence-range', requireAuth('admin'), (req, res) => {
  ...
  const sql = `
    SELECT
      ...
      SUM(CASE WHEN cr.audit_status = 'approved' THEN 1 ELSE 0 END) AS total_cleaned
    FROM car_assignments ca
    LEFT JOIN cleaning_reports cr
      ON ca.reg_no = cr.reg_no
      AND ca.hub_location = cr.hub_location
      AND DATE(cr.submission_date) BETWEEN DATE(?) AND DATE(?)
    WHERE DATE(ca.assigned_at) BETWEEN DATE(?) AND DATE(?)
  `;
  db.get(sql, [startDate, endDate, startDate, endDate], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
    const assigned = row.total_assigned || 0;
    const cleaned  = row.total_cleaned  || 0;
    res.json({
      start: startDate,
      end:   endDate,
      assigned,
      cleaned,
      adherence: assigned ? Math.round(cleaned/assigned*100) : 0
    });
  });
});
...
app.get('/hub-adherence-summary', requireAuth('admin'), (req, res) => {
  const sql = `
    SELECT
      ...
      ROUND(
        (COUNT(DISTINCT CASE WHEN cr.audit_status='approved' THEN cr.reg_no END) * 100.0) /
        NULLIF(COUNT(DISTINCT ca.reg_no), 0),
      1) AS adherence_rate
    FROM car_assignments ca
    LEFT JOIN cleaning_reports cr
      ON ca.reg_no = cr.reg_no AND ca.hub_location = cr.hub_location
    GROUP BY ca.hub_location
    -- REMOVE any HAVING clause filtering adherence
...
// Date‐filtered per‐hub adherence
app.get('/hub-adherence-summary-range', requireAuth('admin'), (req, res) => {
  const { start, end } = req.query;
  ...
  const sql = `
    SELECT
      ca.hub_location,
      COUNT(*) AS total_assigned,
      COUNT(CASE WHEN cr.audit_status='approved' THEN 1 END) AS total_cleaned,
      ROUND(
        COUNT(CASE WHEN cr.audit_status='approved' THEN 1 END) *100.0 / COUNT(*),
      1) AS adherence_rate
    FROM car_assignments ca
    LEFT JOIN cleaning_reports cr
      ON ca.reg_no = cr.reg_no
...
app.get('/yard-manager-videos', requireAuth('yard_manager'), (req, res) => {
    ...
    `, [req.session.userId], (err, rows) => {
        if (err) {
            console.error('Error fetching yard manager videos:', err);
            return res.status(500).json({ error: err.message });
        }
        res.json(rows || []);
    });
...
app.get('/yard-manager-audits', requireAuth('yard_manager'), (req, res) => {
    db.all(`
        SELECT 
            ...
            h.name as hub_name, h.location as hub_location_full,
            CASE 
                WHEN cr.audit_status = 'approved' THEN 'Approved'
                WHEN cr.audit_status = 'rejected' THEN 'Rejected'
                ELSE 'Pending'
            END as audit_status_display
        FROM cleaning_reports cr
        JOIN hubs h ON cr.hub_location = h.name
        JOIN hub_assignments ha ON h.id = ha.hub_id
        WHERE ha.user_id = ? AND ha.role = 'yard_manager'
    ...
    `, [req.session.userId], (err, rows) => {
        if (err) {
            console.error('Error fetching yard manager audits:', err);
            return res.status(500).json({ error: err.message });
        }
        res.json(rows || []);
    });
...
app.post('/admin-assign-yard-manager', requireAuth('admin'), (req, res) => {
  const { email, hub_id } = req.body;
  if (!email || !hub_id) return res.status(400).json({ error: 'Email and hub_id required' });

  // Upsert user
  db.run(`
    INSERT INTO users (email, role)
    VALUES (?, 'yard_manager')
    ON CONFLICT(email) DO UPDATE SET role='yard_manager'
  `, [email], function(err) {
    if (err) return res.status(500).json({ error: err.message });
    const userId = this.lastID || 
      // fetch existing ID if lastID is 0
      null;
    // fetch ID
    db.get(`SELECT id FROM users WHERE email=?`, [email], (err, row) => {
      ...
      // assign to hub
      db.run(`
        INSERT OR REPLACE INTO hub_assignments (hub_id, user_id, role)
        VALUES (?, ?, 'yard_manager')
      `, [hub_id, id], err => {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ success: true, message: 'Yard manager assigned' });
      });
    });
  });
});
...
// REPLACE your existing /hubs endpoint with this:
// app.get('/hubs', requireAuth('admin'), (req, res) => {
//     db.all(`
//         SELECT DISTINCT id, name, location, sheet_id
//         FROM hubs
//         WHERE name IS NOT NULL
//         AND name NOT LIKE 'UP%'
//         AND name NOT LIKE 'PB%'  
...
//     `, [], (err, rows) => {
//         if (err) return res.status(500).json({ error: err.message });
//         console.log(`Returning ${rows.length} valid hubs:`, rows);
//         res.json(rows);
//     });
// });
app.get('/hubs', requireAuth('admin'), (req, res) => {
  db.all(
    `SELECT id, name FROM hubs WHERE name <> '' ORDER BY name`,
    [], (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(rows);
    }
...
// Clean up corrupted data - ONE TIME USE
app.post('/fix-corrupted-hubs', requireAuth('admin'), (req, res) => {
    db.serialize(() => {
        db.run('DELETE FROM hubs', function(err) {
            if (err) return res.status(500).json({ error: err.message });
            
            const correctHubs = [
                ['Agra Hub', 'Agra', 'agra_hub'],
                ['Chandigarh Hub', 'Chandigarh', 'chandigarh_hub']
            ];
            
            let insertedCount = 0;
            correctHubs.forEach(([name, location, sheet_id]) => {
                db.run('INSERT INTO hubs (name, location, sheet_id) VALUES (?, ?, ?)',
                    [name, location, sheet_id], function(err) {
                    if (err) console.error('Error inserting hub:', err);
                    else {
                        console.log(`Inserted hub: ${name}`);
                        insertedCount++;
                        if (insertedCount === correctHubs.length) {
                            res.json({ 
                                success: true, 
                                message: `Fixed hub data. Inserted ${correctHubs.length} hubs.` 
                            });
                        }
                    }
                });
...
// Yard Manager Statistics (missing endpoint)
app.get('/yard-stats', requireAuth('yard_manager'), (req, res) => {
    db.serialize(() => {
        let stats = {};
        ...
        `, [req.session.userId], (err, row) => {
            ...
            `, [req.session.userId], (err, row) => {
                ...
                db.get(`
                    ...
                    JOIN hub_assignments ha ON h.id = ha.hub_id
                    WHERE ha.user_id = ? AND ha.role = 'yard_manager'
                    AND DATE(cr.submission_date) >= DATE('now', '-7 days')
                `, [req.session.userId], (err, row) => {
                    stats.week_uploads = err ? 0 : (row.count || 0);
                    ...
                    db.get(`
                        SELECT COUNT(*) as count FROM car_assignments ca
                        LEFT JOIN cleaning_reports cr ON ca.reg_no = cr.reg_no
                        JOIN hubs h ON ca.hub_location = h.name
                        JOIN hub_assignments ha ON h.id = ha.hub_id
                        WHERE ha.user_id = ? AND ha.role = 'yard_manager'
                        AND cr.id IS NULL AND julianday('now') - julianday(ca.assigned_at) > 2
                    `, [req.session.userId], (err, row) => {
...
// Update reason for car without videos
app.post('/update-car-reason', requireAuth('yard_manager'), (req, res) => {
    const { reg_no, new_reason } = req.body;
    
    if (!reg_no || !new_reason) {
        return res.status(400).json({ error: 'Car registration number and reason are required' });
    }
    
    // Verify yard manager has access to this car's hub
    db.get(`
        SELECT ca.id 
        FROM car_assignments ca
        JOIN hub_assignments ha ON ca.hub_location = (SELECT name FROM hubs WHERE id = ha.hub_id)
        WHERE ca.reg_no = ? AND ha.user_id = ? AND ha.role = 'yard_manager'
    `, [reg_no, req.session.userId], (err, car) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!car) return res.status(403).json({ error: 'Access denied to this car assignment' });
        
        // Update the reason
        db.run(`UPDATE car_assignments SET reason = ? WHERE reg_no = ?`, 
            [new_reason, reg_no], function(err) {
            if (err) return res.status(500).json({ error: err.message });
...
// Get video file paths for viewing
app.get('/get-video-paths/:reportId', requireAuth('yard_manager'), (req, res) => {
    const reportId = req.params.reportId;
    
    db.get(`
        SELECT cr.id, cr.reg_no, cr.hub_location, cr.exterior_video_path, cr.interior_video_path,
               cr.submission_date, cr.user_email as ground_worker
        FROM cleaning_reports cr
        JOIN hubs h ON cr.hub_location = h.name
        JOIN hub_assignments ha ON h.id = ha.hub_id
        WHERE cr.id = ? AND ha.user_id = ? AND ha.role = 'yard_manager'
    `, [reportId, req.session.userId], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!row) return res.status(404).json({ error: 'Video report not found or access denied' });
        
        // Check if video files exist
        const fs = require('fs');
        const exteriorExists = row.exterior_video_path && fs.existsSync(row.exterior_video_path);
        const interiorExists = row.interior_video_path && fs.existsSync(row.interior_video_path);
...
// Serve video files for yard manager viewing
app.get('/view-video/:reportId/:type', requireAuth('yard_manager'), (req, res) => {
    const { reportId, type } = req.params;
    
    db.get(`
        SELECT cr.exterior_video_path, cr.interior_video_path
        FROM cleaning_reports cr
        JOIN hubs h ON cr.hub_location = h.name
        JOIN hub_assignments ha ON h.id = ha.hub_id
        WHERE cr.id = ? AND ha.user_id = ? AND ha.role = 'yard_manager'
    `, [reportId, req.session.userId], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        if (!row) return res.status(404).json({ error: 'Access denied' });
        
        const videoPath = type === 'exterior' ? row.exterior_video_path : row.interior_video_path;
        
        if (!videoPath || !require('fs').existsSync(videoPath)) {
            return res.status(404).json({ error: 'Video file not found' });
        }
        
        res.sendFile(require('path').resolve(videoPath));
    });
});
// Hub adherence summary endpoint - MISSING FROM YOUR SERVER
// app.get('/hub-adherence-summary', requireAuth('admin'), (req, res) => {
//     db.all(`
//         SELECT 
//             ca.hub_location,
...
//                 WHEN cr.audit_status = 'approved' 
//                 THEN cr.reg_no 
//             END) as total_cleaned,
//             ROUND(
//                 (COUNT(DISTINCT CASE WHEN cr.audit_status = 'approved' THEN cr.reg_no END) * 100.0) / 
//                 COUNT(DISTINCT ca.reg_no), 1
//             ) as adherence_rate
//         FROM car_assignments ca
//         LEFT JOIN cleaning_reports cr ON ca.reg_no = cr.reg_no 
//             AND ca.hub_location = cr.hub_location
//         GROUP BY ca.hub_location
//         HAVING adherence_rate > 3
//         ORDER BY adherence_rate DESC
//     `, [], (err, rows) => {
//         if (err) {
//             console.error('Hub adherence summary error:', err);
//             return res.status(500).json({ error: err.message });
//         }
        
//         console.log('Hub adherence summary data:', rows);
//         res.json(rows || []);
//     });
// });
// Hub adherence summary - FIXED VERSION
// app.get('/hub-adherence-summary', requireAuth('admin'), (req, res) => {
//   db.all(`
//     SELECT 
//       ca.hub_location,
...
// New: Monthly adherence statistics (ADMIN ONLY)
app.get('/monthly-stats', requireAuth('admin'), (req, res) => {
  // Example response structure: [{ month: '2025-07', assigned: 50000, cleaned: 40000 }, ...]
  const query = `
    SELECT
    ...
    LEFT JOIN cleaning_reports cr
      ON ca.reg_no = cr.reg_no
      AND ca.hub_location = cr.hub_location
      AND STRFTIME('%Y-%m', ca.assigned_at) = STRFTIME('%Y-%m', cr.submission_date)
    GROUP BY month
    ORDER BY month DESC
  `;
  db.all(query, [], (err, rows) => {
    ...
    // Fill missing months if you like, but this returns only months with data
    res.json(rows.map(r => ({
      month: r.month,
      assigned: r.total_assigned,
      cleaned: r.total_cleaned,
      adherence: r.total_assigned
...
/* -----------------------------------------------------------
   Date-range adherence  (admin only)
   GET /adherence-range?start=YYYY-MM-DD&end=YYYY-MM-DD
   If either start or end is missing it defaults to TODAY.
------------------------------------------------------------ */
app.get('/adherence-range', requireAuth('admin'), (req, res) => {
  const start = req.query.start || 'now';
  const end   = req.query.end   || 'now';

  /* one query, all hubs combined; if you want per-hub,
     add  GROUP BY ca.hub_location  and aggregate in JS */
  const sql = `
    SELECT
      COUNT(*) AS total_assigned,
      SUM(
        CASE WHEN cr.audit_status = 'approved' THEN 1 ELSE 0 END
      ) AS total_cleaned
    FROM car_assignments  ca
    LEFT JOIN cleaning_reports cr
      ON  ca.reg_no       = cr.reg_no
  ...
  db.get(sql, [start, end, start, end], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
